...
class ExperianApiWebClientTest {

    @Test
    void inquireCreditScory_success() {
        ExchangeFunction exchangeFunction = mock(ExchangeFunction.class);
        WebClient webClient = WebClient.builder().exchangeFunction(exchangeFunction).build();
        ExperianApiWebClient experianApiWebClient= new ExperianApiWebClient(webClient);
        ...
        Mono<Integer> responseMono = experianApiWebClient.inquireCreditScory(request);
        StepVerifier
                .create(responseMono)
                .expectNextCount(1)
                .expectComplete()
